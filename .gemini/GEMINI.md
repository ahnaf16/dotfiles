...
* **Logging:** Use the `chirp` package instead of `print`.
...
* **JSON Serialization:** Use `dart_mappable` for
...
* **Structured Logging:** Use the `Chirp` from `chirp` package for
  ...
  // For simple messages
  Chirp.info('User logged in successfully.');
...
* **Universal images:** if `universal_image` package exists use this for all type of images.
* **Asset Generation:** Use `flutter_gen` package to generate asset strings.
